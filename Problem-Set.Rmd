...
decision = ifelse(y_1 > y_0 - 1000, 1, 0)

df = data.frame(y_0, y_1, decision)
```
...
```{R}
sample_ate = mean(df[df$decision == 1, "y_1"]) - mean(df[df$decision == 0, "y_0"])
sample_ate
...
```{R}
#2SLS Regression
feols(data = df, y ~ treatment | decision)
```
...
table$block_variance = table$prob_T*(1-table$prob_T)
#Number of treated individuals in each block
table$treated = table$N*table$prob_T
#Total treatment probability
treatedprob = sum(table$treated)/sum(table$N)
#Total variance
total_var = treatedprob*(1-treatedprob)
#Weights for each block
table$weights = table$block_variance*table$N/(sum(table$N)*total_var)


#sum(table$N/sum(table$N))
#sum(table$block_variance/total_var)
...
$$
Var(\hat\Delta) = N*p*(1-p) = 10000*0.729*(1-0.729) \\
Var(\hat\Delta) = \dfrac{\sigma^2}{N*Var(T_i)} \\
\sigma^2 = Var(\hat\Delta)*N*Var(T_i)
$$
...
$$
\delta = (t_{\alpha/2} + t_{\beta})*\sqrt{\dfrac{\sigma^2}{N*Var(T_i)}} \\
\delta = (t_{\alpha/2} + t_{\beta})*\sqrt{\dfrac{Var(\hat\Delta)*N*Var(T_i)}{N*Var(T_i)}} \\
\delta = (t_{\alpha/2} + t_{\beta})*\sqrt{\dfrac{Var(\hat\Delta)*N*Var(T_i)}{N*Var(T_i)}} \\
\delta = (t_{\alpha/2} + t_{\beta})*\sqrt{Var(\hat\Delta)} \\
$$
...
where $t_{\alpha/2} = 1.96$, $t_{\beta} = 0.84$, $N = 10000$, $Var(T_i) = p*(1-p) = 0.5*0.5$, $n = \dfrac{10000}{118}$, $\rho = 0.2$ and $\sigma^2 = ?$.
...
For both of our answers, as our $R^2$ goes up, then the square rooted term becomes smaller. This means that we can detect a smaller $\delta$. This is good because before we may have detected an effect of 10, but now we could detect an effect of 8.
